...
import { throttleByRaf } from 'element-plus/es/utils';
  ...
  //跳转到评论页面
  const showReviews=(scope)=>{
    console.log("qweasdzxc")
    app.appContext.config.globalProperties.$reviewId=scope.row.id
    router.push('/review')
  }
                ...
                <el-table :data="products" border style="width: 100%">
                  ...
                  <el-table-column prop="image" label="Image">
                      <template #default="scope">
                          ...
                          <template #content>主要类别：{{scope.row.maincategory}}<br />平均评分：{{ scope.row.averagerating }} <br/>评分：{{ scope.row.ratingnumber }}<br/>特征：{{ scope.row.features }} <br/>商店：{{ scope.row.store }} </template>
                          <el-image @click="showReviews(scope)" :src="scope.row.image"></el-image>
                          </el-tooltip>
                      </template>
                  </el-table-column>
