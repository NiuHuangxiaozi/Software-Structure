...
public class JD implements PosDB {
    ...
    private boolean isGetProducts;

    public  JD(CategoryRepository cate,ProductRepository pro){
    ...
    public List<Product> getProducts() {
        ...
        try {
            if (this.isGetProducts==false){
                System.out.print("load from JD!");
                products = parseJD("Java");
                product_repository.saveAll(products);
                this.isGetProducts=true;
            }
            else{
                System.out.print("load from H2 Database!");
