...
<script lang="ts" setup>
  ...
  import { getAllorders,changeNumberOfProduct ,deleteorder,changeOrderState,getAlldelis} from '@/utils/api';
  ...
  const getDelis=async function()
  {
    let result=await getAlldelis()
    for(var index=0;index<result.length;index++)
    {
      deliveries.value.push(result[index])
    }
                  ...
                  <el-table-column  label="opeation" style="width: 70%">
                      <template #default="{ row }">
                        ...
                        >
                              ...
                              <template #content> 
                                  <div  v-for="item in deliveries" :key="item">
                                        <span v-if="item.orderId==row.orderId">
                                          运单号：{{item.deliveryId}} <br/> 发货区间：{{item.logistics}} <br/> 商品描述：{{item.describe}}
                                        </span>
                                  </div>
                              </template>
