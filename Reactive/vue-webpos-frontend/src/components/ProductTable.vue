...
<script lang="ts" setup>
  ...
  import { getAllproducts,getProduct,changeNumberOfProduct,addOrder,getProductByName } from '@/utils/api';
  import {ref} from 'vue'
...
import { throttleByRaf } from 'element-plus/es/utils';
  ...
  let orderId = app.appContext.config.globalProperties.$orderId
  ...
  const charge=async ()=>{
    ...
    //准备OrderId
    let OrderId="JD"+orderId.toString()
    app.appContext.config.globalProperties.$orderId=orderId+1
    console.log(app.appContext.config.globalProperties.$orderId)
...
const searchProduct=async ()=>{
    ...
    if (productName.value==''){
      ...
      let result=await getProductByName(productName.value)
...
<template>
  <div class="mt-4">
    ...
    >
      <template #append>
        <el-button @click="searchProduct":icon="Search" />
      </template>
    </el-input>
  </div>
